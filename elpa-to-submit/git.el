...
(defun git-diff-file-idiff-orig ()
  "Perform an interactive diff on the current file."
  (interactive)
  (let ((files (git-marked-files-state 'added 'deleted 'modified)))
    (unless (eq 1 (length files))
      (error "Cannot perform an interactive diff on multiple files."))
    (let* ((filename (last (git-get-filenames files)))
		   (ediff-after-quit-destination-buffer (current-buffer))
           (my-buffer (find-file-noselect filename))
           (base-buff (git-run-command-buffer (concat filename ".~HEAD~") "cat-file" "blob" (concat "HEAD:" filename)))
		   (git-transient-buffers (list base-buff ))
		   (startup-hook '(git-ediff-startup-hook)))
      (ediff-buffers base-buff my-buffer startup-hook))) ;; cambie el orden para que HEAD aparezca en el lado izquierdo del buffer, como en psvn
...
(defun git-diff-file-idiff ()
  "Perform an interactive diff on the current file."
  (interactive)
  (let* ((info-fname (ewoc-data (ewoc-locate git-status)))
		 (filename (git-fileinfo->name info-fname))
		 (ediff-after-quit-destination-buffer (current-buffer))
		 (my-buffer (find-file-noselect filename))
		 (base-buff (git-run-command-buffer (concat filename ".~HEAD~") "cat-file" "blob" (concat "HEAD:" filename)))
		 (git-transient-buffers (list base-buff ))
...
(defun git-ediff-startup-hook ()
  (add-hook 'ediff-after-quit-hook-internal
        `(lambda ()
           (git-ediff-exit-hook
        ',ediff-after-quit-destination-buffer ',git-transient-buffers))
        nil 'local))

(defun git-ediff-exit-hook (git-buf tmp-bufs)
  ;; kill the temp buffers (and their associated windows)
  (dolist (tb tmp-bufs)
    (when (and tb (buffer-live-p tb) ) ;; (not (buffer-modified-p tb)) <- quite este check dado que git.el crea un buffer temporal con el contenido de *~HEAD~ y tmp-bufs solo trae el nombre de ese archivo por lo que no se deben de perder datos
      (let ((win (get-buffer-window tb t)))
    (when win (delete-window win))
    (kill-buffer tb))))
  ;; switch back to the *git* buffer
  (when (and git-buf (buffer-live-p git-buf)
         (not (get-buffer-window git-buf t)))
    (ignore-errors (switch-to-buffer git-buf))))
...
(unless git-status-mode-map
  (let ((map (make-keymap))
    ...
    (define-key map "E" 'git-diff-file-idiff) ;; como en psvn
    ; the commit submap
